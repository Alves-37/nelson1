...
async def lifespan(app: FastAPI):
    ...
    try:
        async with engine.begin() as conn:
            ...
            # Migração leve para garantir colunas de 'abastecimentos' (produção já pode ter tabela antiga)
            try:
                ...
                # Chaves estrangeiras (em caso de tabelas existentes, usar IF NOT EXISTS no nome evita duplicar)
                await conn.execute(text("""
                    DO $$ BEGIN
                        IF NOT EXISTS (
                ...
                print(f"Aviso: falha ao aplicar migração leve de 'abastecimentos': {mig_e}")
